...
Parse.initialize(
  "b4dljmUQXWU3XkKEn2jShWvcKvX6tK8nIPwMvqEj",
  "485d0YMr2bUyKshc9yphv28kmXsVhKbGF9DKWHQM"
...
async function main() {
  ...
  await mongoose.connect(
    "mongodb+srv://Titan7987:TZPh-CDy982WpNA@cluster0.udqrglp.mongodb.net/?retryWrites=true&w=majority/todolistDB"
  );
}
...
app.post("/delete", function (req, res) {
  ...
  if (list_name === "today") {
    ...
    List.findOneAndUpdate(
      { listName: list_name },
      { $pull: { listItems: { _id: itemToDelete } } },
      function (err, result) {
        if (!err) {
        ...
        List.findOne({ listName: extension }, function (err, foundList) {
          if (!err) {
            res.render("list", {
              ...
              listTitle: _.startCase(extension),
            });
          }
        });
